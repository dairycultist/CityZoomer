...
func _ready() -> void:
	...
	place_room(connector_rooms[0], Node3D.new(), 3)
...
func place_room(room_prefab: PackedScene, from_door: Node3D, max_depth: int) -> void:
	...
	while not doors.is_empty():
		place_room(connector_rooms[1], doors.pop_front(), max_depth - 1)
